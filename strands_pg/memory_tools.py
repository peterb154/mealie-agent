...
def memory_tools(
    ...
    *,
    ...
    if namespace is None and not namespaces:
        raise ValueError("memory_tools requires either namespace=<str> or namespaces={suffix: ns}")
...
def _build_pair(
    ...
    remember_tool = _as_tool(
        remember_fn,
        remember_name,
        (f"Save a durable note{scope_desc}.\n\nArgs:\n    text: The content to remember."),
    )

    recall_tool = _as_tool(
        recall_fn,
        recall_name,
        (
